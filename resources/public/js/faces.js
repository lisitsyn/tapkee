d3.json('data/faces.json', (json) => {
  ...
  const svg = d3.select('#facesPlot').append('svg:svg')
    .attr('width', width)
    .attr('height', height);
  ...
  $('#facesPlot svg circle').each(function() {
    const d = this.__data__;
    const string = d.fname;
    const img = `<img src="img/oks/${string}"/>`;
  ...
  const container = document.getElementById('facesPlot');
