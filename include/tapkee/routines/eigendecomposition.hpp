                                                        ...
                                                        unsigned int skip)
{
    ...
    ArpackGeneralizedSelfAdjointEigenSolver<MatrixType, MatrixType, MatrixOperationType> arpack(
        wm, target_dimension + skip, MatrixOperationType::ARPACK_CODE());
                                                       ...
                                                       unsigned int skip)
{
    ...
    if (solver.info() == Eigen::Success)
    {
        ...
        {
            DenseMatrix selected_eigenvectors =
                solver.eigenvectors().leftCols(target_dimension + skip).rightCols(target_dimension);
            return EigendecompositionResult(selected_eigenvectors,
                                            solver.eigenvalues().segment(skip, skip + target_dimension));
        }
    }
    else
    {
                                                            ...
                                                            unsigned int skip)
{
    ...
    if (eigenOfB.info() == Eigen::Success)
    {
        ...
        {
            DenseMatrix selected_eigenvectors =
                (Y * eigenOfB.eigenvectors()).leftCols(target_dimension + skip).rightCols(target_dimension);
            return EigendecompositionResult(selected_eigenvectors, eigenOfB.eigenvalues());
        }
    }
    else
...
template <> struct eigendecomposition_impl<DenseMatrix>
{
                                    ...
                                    const EigendecompositionStrategy &eigen_strategy, IndexType target_dimension)
    {
        if (strategy.is(HomogeneousCPUStrategy))
        {
            if (eigen_strategy.is(LargestEigenvalues))
                return eigendecomposition_impl_arpack<DenseMatrix, DenseMatrixOperation>(m, target_dimension,
                                                                                         eigen_strategy.skip());
            ...
            if (eigen_strategy.is(SmallestEigenvalues))
                return eigendecomposition_impl_arpack<DenseMatrix, DenseInverseMatrixOperation>(m, target_dimension,
                                                                                                eigen_strategy.skip());
            unsupported();
        }
        ...
        {
            if (eigen_strategy.is(LargestEigenvalues))
                return eigendecomposition_impl_arpack<DenseMatrix, GPUDenseMatrixOperation>(m, target_dimension,
                                                                                            eigen_strategy.skip());
            if (eigen_strategy.is(SquaredLargestEigenvalues))
                                   ...
                                   const EigendecompositionStrategy &eigen_strategy, IndexType target_dimension)
    {
        ...
        {
            if (eigen_strategy.is(LargestEigenvalues))
                return eigendecomposition_impl_dense<DenseMatrix, DenseMatrixOperation>(m, target_dimension,
                                                                                        eigen_strategy.skip());
            if (eigen_strategy.is(SquaredLargestEigenvalues))
                return eigendecomposition_impl_dense<DenseMatrix, DenseMatrixOperation>(m, target_dimension,
                                                                                        eigen_strategy.skip());
            if (eigen_strategy.is(SmallestEigenvalues))
                return eigendecomposition_impl_dense<DenseMatrix, DenseInverseMatrixOperation>(m, target_dimension,
                                                                                               eigen_strategy.skip());
            unsupported();
        }
        unsupported();
                                        ...
                                        const EigendecompositionStrategy &eigen_strategy, IndexType target_dimension)
    {
        ...
        {
            if (eigen_strategy.is(LargestEigenvalues))
                return eigendecomposition_impl_randomized<DenseMatrix, DenseMatrixOperation>(m, target_dimension,
                                                                                             eigen_strategy.skip());
            if (eigen_strategy.is(SquaredLargestEigenvalues))
        ...
        {
            if (eigen_strategy.is(LargestEigenvalues))
                return eigendecomposition_impl_randomized<DenseMatrix, GPUDenseMatrixOperation>(m, target_dimension,
                                                                                                eigen_strategy.skip());
            if (eigen_strategy.is(SquaredLargestEigenvalues))
                                            ...
                                            IndexType target_dimension)
{
    LoggingSingleton::instance().message_info(
        formatting::format("Using the {} eigendecomposition method.", get_eigen_method_name(method)));
#ifdef TAPKEE_WITH_ARPACK
    if (method.is(Arpack))
