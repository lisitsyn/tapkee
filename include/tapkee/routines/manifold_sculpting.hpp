...
inline SparseMatrixNeighborsPair angles_matrix_and_neighbors(const Neighbors &neighbors, const DenseMatrix &data)
{
    ...
    for (IndexType i = 0; i < n_vectors; ++i)
    {
        ...
        for (IndexType j = 0; j < k; ++j)
        {
            ...
            for (IndexType l = 0; l < k; ++l)
            {
                ...
                DenseVector neighbor_to_its_neighbor =
                    data.col(neighbors_of_neighbor[l]) - data.col(current_neighbors[j]);
                current_cos_value = neighbor_to_point.dot(neighbor_to_its_neighbor) /
                                    (neighbor_to_point.norm() * neighbor_to_its_neighbor.norm());
    ...
    return SparseMatrixNeighborsPair(sparse_matrix_from_triplets(sparse_triplets, n_vectors, n_vectors),
                                          ...
                                          const DataForErrorFunc &error_func_data)
{
    ...
    for (IndexType i = 0; i < k; ++i)
    {
        ...
        DenseVector neighbor_to_its_neighbor = data.col(neighbor_of_neighbor) - data.col(neighbor);
         ...
         * original algorithm
         */
        ScalarType weight = (error_func_data.adjusted_points.count(neighbor) == 0) ? 1 : weight_for_adjusted_point;
                              ...
                              IndexType max_iteration, ScalarType squishing_rate)
{
    ...
    SparseMatrixNeighborsPair angles_and_neighbors = angles_matrix_and_neighbors(neighbors, data);
