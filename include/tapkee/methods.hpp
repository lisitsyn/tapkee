  ...
  public:
    ImplementationBase(RandomAccessIterator b, RandomAccessIterator e, KernelCallback k, DistanceCallback d,
        ...
        : parameters(pmap), context(ctx), kernel(k), distance(d), features(f),
          ...
          p_computation_strategy(), p_eigen_method(), p_neighbors_method(), p_eigenshift(), p_traceshift(),
          p_check_connectivity(), p_n_neighbors(), p_width(), p_timesteps(), p_ratio(), p_max_iteration(),
          p_tolerance(), p_n_updates(), p_perplexity(), p_theta(), p_squishing_rate(), p_global_strategy(), p_epsilon(),
          p_target_dimension(), n_vectors(0), current_dimension(0)
    {
        n_vectors = (end - begin);
...
#undef tapkee_method_handle
  ...
  private:
    ...
    TapkeeOutput embedKernelLocallyLinearEmbedding()
    {
        ...
        DenseMatrix embedding = eigendecomposition(p_eigen_method, p_computation_strategy, SmallestEigenvalues,
                                                   weight_matrix, p_target_dimension)
                                    .first;
    ...
    TapkeeOutput embedDiffusionMap()
    {
        ...
        DenseSymmetricMatrix diffusion_matrix = compute_diffusion_matrix(begin, end, distance, p_width);
        EigendecompositionResult decomposition_result = eigendecomposition(
            p_eigen_method, p_computation_strategy, LargestEigenvalues, diffusion_matrix, target_dimension_add);
    ...
    TapkeeOutput embedMultidimensionalScaling()
    {
        ...
        EigendecompositionResult embedding = eigendecomposition(
            p_eigen_method, p_computation_strategy, LargestEigenvalues, distance_matrix, p_target_dimension);
...
#undef MDS_MATRIX_OP
    ...
    TapkeeOutput embedLandmarkMultidimensionalScaling()
    {
        ...
        Landmarks landmarks = select_landmarks_random(begin, end, p_ratio);
        ...
        return TapkeeOutput(triangulate(begin, end, distance, landmarks, landmark_distances_squared,
                                        landmarks_embedding, p_target_dimension),
    ...
    TapkeeOutput embedIsomap()
    {
        ...
        EigendecompositionResult embedding = eigendecomposition(
            p_eigen_method, p_computation_strategy, LargestEigenvalues, shortest_distances_matrix, p_target_dimension);
    ...
    TapkeeOutput embedLandmarkIsomap()
    {
        ...
        if (p_eigen_method.is(Dense))
        {
            ...
            landmarks_embedding = eigendecomposition(p_eigen_method, p_computation_strategy, LargestEigenvalues,
                                                     distance_matrix_sym, p_target_dimension);
        }
        else
        {
            landmarks_embedding = eigendecomposition(p_eigen_method, p_computation_strategy, SquaredLargestEigenvalues,
    ...
    {
        Neighbors neighbors = findNeighborsWith(kernel_distance);
        SparseWeightMatrix weight_matrix = hessian_weight_matrix(begin, end, neighbors, kernel, p_target_dimension);
        return TapkeeOutput(eigendecomposition(p_eigen_method, p_computation_strategy, SmallestEigenvalues,
                                               weight_matrix, p_target_dimension)
                                .first,
                            unimplementedProjectingFunction());
    }
    ...
    {
        Neighbors neighbors = findNeighborsWith(plain_distance);
        Laplacian laplacian = compute_laplacian(begin, end, neighbors, distance, p_width);
        return TapkeeOutput(generalized_eigendecomposition(p_eigen_method, p_computation_strategy, SmallestEigenvalues,
                                                           laplacian.first, laplacian.second, p_target_dimension)
                                .first,
                            unimplementedProjectingFunction());
    }
    ...
    TapkeeOutput embedLocalityPreservingProjections()
    {
        ...
        EigendecompositionResult projection_result = generalized_eigendecomposition(
            p_eigen_method, p_computation_strategy, SmallestEigenvalues, eigenproblem_matrices.first,
            eigenproblem_matrices.second, p_target_dimension);
        DenseVector mean_vector = compute_mean(begin, end, features, current_dimension);
        tapkee::ProjectingFunction projecting_function(
            new tapkee::MatrixProjectionImplementation(projection_result.first, mean_vector));
        return TapkeeOutput(project(projection_result.first, mean_vector, begin, end, features, current_dimension),
    ...
    TapkeeOutput embedPCA()
    {
        ...
        EigendecompositionResult projection_result = eigendecomposition(
            p_eigen_method, p_computation_strategy, LargestEigenvalues, centered_covariance_matrix, p_target_dimension);
        tapkee::ProjectingFunction projecting_function(
            new tapkee::MatrixProjectionImplementation(projection_result.first, mean_vector));
        return TapkeeOutput(project(projection_result.first, mean_vector, begin, end, features, current_dimension),
                            projecting_function);
    ...
    {
        DenseMatrix projection_matrix = gaussian_projection_matrix(current_dimension, p_target_dimension);
        DenseVector mean_vector = compute_mean(begin, end, features, current_dimension);
    ...
    TapkeeOutput embedKernelPCA()
    {
        ...
        EigendecompositionResult embedding = eigendecomposition(
            p_eigen_method, p_computation_strategy, LargestEigenvalues, centered_kernel_matrix, p_target_dimension);
        for (IndexType i = 0; i < static_cast<IndexType>(p_target_dimension); i++)
            embedding.first.col(i).array() *= sqrt(embedding.second(i));
        return TapkeeOutput(embedding.first, unimplementedProjectingFunction());
    }
    ...
    TapkeeOutput embedStochasticProximityEmbedding()
    {
        ...
        return TapkeeOutput(spe_embedding(begin, end, distance, neighbors, p_target_dimension, p_global_strategy,
                                          p_tolerance, p_n_updates, p_max_iteration),
                            unimplementedProjectingFunction());
    }
    ...
    TapkeeOutput embedFactorAnalysis()
    {
        ...
        return TapkeeOutput(project(begin, end, features, current_dimension, p_max_iteration, p_epsilon,
                                    p_target_dimension, mean_vector),
                            unimplementedProjectingFunction());
    }
    ...
    TapkeeOutput embedManifoldSculpting()
    {
        ...
        manifold_sculpting_embed(begin, end, embedding, p_target_dimension, neighbors, distance, p_max_iteration,
                                 p_squishing_rate);
