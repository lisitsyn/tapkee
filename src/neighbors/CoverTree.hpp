 ...
 */
template<class Point, class SimilarityCallback>
class CoverTree
{
    /**
...
template<class Point, class SimilarityCallback>
CoverTree<Point,SimilarityCallback>::CoverTree(const double& maxDist,
...
template<class Point, class SimilarityCallback>
CoverTree<Point,SimilarityCallback>::~CoverTree()
{
...
template<class Point, class SimilarityCallback>
std::vector<typename CoverTree<Point,SimilarityCallback>::CoverTreeNode*>
CoverTree<Point,SimilarityCallback>::kNearestNodes(const Point& p, const unsigned int& k) const
{
    ...
    double maxDist = SimilarityCallback(p,_root->getPoint());
    ...
    for(int level = _maxLevel; level>=_minLevel;level--) {
        ...
        for(int i=0; i<size; i++) {
            ...
            for(it2=children.begin(); it2!=children.end(); ++it2) {
                double d = SimilarityCallback(p,(*it2)->getPoint());
...
template<class Point, class SimilarityCallback>
bool CoverTree<Point,SimilarityCallback>::insert_rec(const Point& p,
                                  const std::vector<distNodePair>& Qi,
                                  const int& level)
{
...
template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::remove_rec(const Point& p,
                                  std::map<int,std::vector<distNodePair> >& coverSets,
                                  int level,
                                  bool& multi)
{
    ...
    for(it=Qi.begin();it!=Qi.end();++it) {
        ...
        typename std::vector<CoverTreeNode*>::const_iterator it2;
        for(it2=children.begin();it2!=children.end();++it2) {
            dist = SimilarityCallback(p,(*it2)->getPoint());
            if(dist<minDist) {
                minDist = dist;
                minNode = *it2;
        ...
        for(it=children.begin();it!=children.end();++it) {
            ...
            while(true) {
                ...
                for(it2=Q.begin();it2!=Q.end();++it2) {
                    double d = SimilarityCallback(q,it2->second->getPoint());
                    if(d<minDQ) {
                        minDQ = d;
                ...
                minDQ=DBL_MAX;
                if(br) break;
                Q.push_back(make_pair(SimilarityCallback(*it,p),*it));
                i++;
                sep = pow(base,i);
            }
...
template<class Point, class SimilarityCallback>
std::pair<double, typename CoverTree<Point,SimilarityCallback>::CoverTreeNode*>
CoverTree<Point,SimilarityCallback>::distance(const Point& p,
                           const std::vector<CoverTreeNode*>& Q)
{
    double minDist = DBL_MAX;
    ...
    for(it=Q.begin();it!=Q.end();++it) {
        double dist = SimilarityCallback(p,(*it)->getPoint());
        if(dist < minDist) {
            minDist = dist;
            minNode = *it;
        }
...
template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::insert(const Point& newPoint)
{
    if(_root==NULL) {
    ...
    if(SimilarityCallback(newPoint,n->getPoint())==0.0) {
        n->addPoint(newPoint);
    } else {
        ...
        insert_rec(newPoint,
                   std::vector<distNodePair>
                   (1,make_pair(SimilarityCallback(_root,newPoint),_root)),
                   _maxLevel);
    }
}

template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::remove(const Point& p)
{
    ...
    std::map<int, std::vector<distNodePair> > coverSets;
    coverSets[_maxLevel].push_back(make_pair(SimilarityCallback(_root,p),_root));
    if(removingRoot)
        coverSets[_maxLevel].push_back(make_pair(SimilarityCallback(newRoot,p),newRoot));
    bool multi = false;
...
template<class Point, class SimilarityCallback>
std::vector<Point> CoverTree<Point,SimilarityCallback>::kNearestNeighbors(const Point& p,
...
template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::print() const
{
...
template<class Point, class SimilarityCallback>
typename CoverTree<Point,SimilarityCallback>::CoverTreeNode* CoverTree<Point,SimilarityCallback>::getRoot() const
{
    return _root;
}

template<class Point, class SimilarityCallback>
CoverTree<Point,SimilarityCallback>::CoverTreeNode::CoverTreeNode(const Point& p) {
    _points.push_back(p);
}

template<class Point, class SimilarityCallback>
std::vector<typename CoverTree<Point,SimilarityCallback>::CoverTreeNode*>
CoverTree<Point,SimilarityCallback>::CoverTreeNode::getChildren(int level) const
{
...
template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::CoverTreeNode::addChild(int level, CoverTreeNode* p)
{
    _childMap[level].push_back(p);
}

template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::CoverTreeNode::removeChild(int level, CoverTreeNode* p)
{
...
template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::CoverTreeNode::addPoint(const Point& p)
{
...
template<class Point, class SimilarityCallback>
void CoverTree<Point,SimilarityCallback>::CoverTreeNode::removePoint(const Point& p)
{
...
template<class Point, class SimilarityCallback>
double CoverTree<Point,SimilarityCallback>::CoverTreeNode::distance(const CoverTreeNode& p) const
{
    return SimilarityCallback(_points[0],p.getPoint());
}
 
template<class Point, class SimilarityCallback>
bool CoverTree<Point,SimilarityCallback>::CoverTreeNode::isSingle() const
{
    return _points.size() == 1;
}

template<class Point, class SimilarityCallback>
bool CoverTree<Point,SimilarityCallback>::CoverTreeNode::hasPoint(const Point& p) const
{
    return find(_points.begin(), _points.end(), p) != _points.end();
}

template<class Point, class SimilarityCallback>
const Point& CoverTree<Point,SimilarityCallback>::CoverTreeNode::getPoint() const { return _points[0]; }

template<class Point, class SimilarityCallback>
std::vector<typename CoverTree<Point,SimilarityCallback>::CoverTreeNode*>
CoverTree<Point,SimilarityCallback>::CoverTreeNode::getAllChildren() const
{
    std::vector<CoverTreeNode*> children;
...
template<class Point, class SimilarityCallback>
bool CoverTree<Point,SimilarityCallback>::isValidTree() const {
    if(_numNodes==0)
        return _root==NULL;
    ...
    for(int i=_maxLevel;i>_minLevel;i--) {
        ...
        for(it=nodes.begin(); it!=nodes.end(); ++it) {
            for(it2=nodes.begin(); it2!=nodes.end(); ++it2) {
                double dist=SimilarityCallback((*it),(*it2)->getPoint());
        ...
        for(it=nodes.begin(); it!=nodes.end(); ++it) {        
            ...
            for(it2=children.begin(); it2!=children.end(); ++it2) {
                double dist = SimilarityCallback((*it2),(*it)->getPoint());
                if(dist>sep) {
