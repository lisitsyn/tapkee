...
#define INPUT_FILE_KEYWORD "input-file"
    opt.add("", 0, 1, 0, "Input file", OPT_PREFIX "i", OPT_LONG_PREFIX INPUT_FILE_KEYWORD);
#define TRANSPOSE_INPUT_KEYWORD "transpose-input"
    opt.add("", 0, 0, 0, "Transpose input file if set", OPT_LONG_PREFIX TRANSPOSE_INPUT_KEYWORD);
#define TRANSPOSE_OUTPUT_KEYWORD "transpose-output"
    opt.add("", 0, 0, 0, "Transpose output file if set", OPT_LONG_PREFIX TRANSPOSE_OUTPUT_KEYWORD);
#define OUTPUT_FILE_KEYWORD "output-file"
    opt.add("", 0, 1, 0, "Output file", OPT_PREFIX "o", OPT_LONG_PREFIX OUTPUT_FILE_KEYWORD);
#define OUTPUT_PROJECTION_MATRIX_FILE_KEYWORD "output-projection-matrix-file"
    opt.add("", 0, 1, 0, "Output file for projection matrix", OPT_PREFIX "opmat",
...
#define OUTPUT_PROJECTION_MEAN_FILE_KEYWORD "output-projection-mean-file"
    opt.add("", 0, 1, 0, "Output file for mean of data", OPT_PREFIX "opmean",
            OPT_LONG_PREFIX OUTPUT_PROJECTION_MEAN_FILE_KEYWORD);
#define DELIMITER_KEYWORD "delimiter"
    opt.add("", 0, 1, 0, "Delimiter", OPT_PREFIX "d", OPT_LONG_PREFIX DELIMITER_KEYWORD);
#define HELP_KEYWORD "help"
    opt.add("", 0, 0, 0, "Display help", OPT_PREFIX "h", OPT_LONG_PREFIX HELP_KEYWORD);
#define BENCHMARK_KEYWORD "benchmark"
    opt.add("", 0, 0, 0, "Output benchmark information", OPT_LONG_PREFIX BENCHMARK_KEYWORD);
#define VERBOSE_KEYWORD "verbose"
    opt.add("", 0, 0, 0, "Output more information", OPT_LONG_PREFIX VERBOSE_KEYWORD);
#define DEBUG_KEYWORD "debug"
    opt.add("", 0, 0, 0, "Output debug information", OPT_LONG_PREFIX DEBUG_KEYWORD);
#define METHOD_KEYWORD "method"
    opt.add(
        ...
        OPT_PREFIX "m", OPT_LONG_PREFIX METHOD_KEYWORD);
...
#endif
        ".",
        OPT_PREFIX "nm", OPT_LONG_PREFIX NEIGHBORS_METHOD_KEYWORD);
...
#endif
        "randomized, dense.",
        OPT_PREFIX "em", OPT_LONG_PREFIX EIGEN_METHOD_KEYWORD);
...
#endif
            "cpu.",
            OPT_PREFIX "cs", OPT_LONG_PREFIX COMPUTATION_STRATEGY_KEYWORD);
#define TARGET_DIMENSION_KEYWORD "target-dimension"
    opt.add("2", 0, 1, 0, "Target dimension (default 2)", OPT_PREFIX "td", OPT_LONG_PREFIX TARGET_DIMENSION_KEYWORD);
#define NUM_NEIGHBORS_KEYWORD "num-neighbors"
    opt.add("10", 0, 1, 0, "Number of neighbors (default 10)", OPT_PREFIX "k", OPT_LONG_PREFIX NUM_NEIGHBORS_KEYWORD);
#define GAUSSIAN_WIDTH_KEYWORD "gaussian-width"
    opt.add("1.0", 0, 1, 0, "Width of gaussian kernel (default 1.0)", OPT_PREFIX "gw",
            OPT_LONG_PREFIX GAUSSIAN_WIDTH_KEYWORD);
#define TIMESTEPS_KEYWORD "timesteps"
    opt.add("1", 0, 1, 0, "Number of timesteps for diffusion map (default 1)", OPT_LONG_PREFIX TIMESTEPS_KEYWORD);
#define SPE_LOCAL_KEYWORD "spe-local"
    opt.add("0", 0, 0, 0, "Local strategy in SPE (default global)", OPT_LONG_PREFIX SPE_LOCAL_KEYWORD);
...
#define SPE_TOLERANCE_KEYWORD "spe-tolerance"
    opt.add("1e-5", 0, 1, 0, "Tolerance for SPE (default 1e-5)", OPT_LONG_PREFIX SPE_TOLERANCE_KEYWORD);
#define SPE_NUM_UPDATES_KEYWORD "spe-num-updates"
    opt.add("100", 0, 1, 0, "Number of SPE updates (default 100)", OPT_LONG_PREFIX SPE_NUM_UPDATES_KEYWORD);
#define MAX_ITERS_KEYWORD "max-iters"
    opt.add("1000", 0, 1, 0, "Maximum number of iterations (default 1000)", OPT_LONG_PREFIX MAX_ITERS_KEYWORD);
#define FA_EPSILON_KEYWORD "fa-epsilon"
    opt.add("1e-5", 0, 1, 0, "FA convergence criterion (default 1e-5)", OPT_LONG_PREFIX FA_EPSILON_KEYWORD);
...
#define SNE_THETA_KEYWORD "sne-theta"
    opt.add("0.5", 0, 1, 0, "Theta for the t-SNE algorithm (default 0.5)", OPT_LONG_PREFIX SNE_THETA_KEYWORD);
