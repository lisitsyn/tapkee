d3.json('data/elections.json', (json) => {
  const width = 400;
  const height = 400;
  const margin = 50;
  ...
  const svg = d3.select('#electionsPlot').append('svg:svg')
    .attr('width', width)
    .attr('height', height);
  ...
  $('#electionsPlot svg circle').each(function() {
    const d = this.__data__;
    const string = d.region;
  ...
  const container = document.getElementById('electionsPlot');
