d3.json('data/synthfaces.json', (json) => {
  const width = 400;
  const height = 400;
  const margin = 50;
  ...
  const svg = d3.select('#synthfacesPlot').append('svg:svg')
    .attr('width', width)
    .attr('height', height);
  ...
  $('#synthfacesPlot svg circle').each(function() {
    const d = this.__data__;
    const string = d.fname;
    const img = `<img src="img/synthfaces/${string}"/>`;
  ...
  const container = document.getElementById('synthfacesPlot');
